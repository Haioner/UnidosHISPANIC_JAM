...
public class DialogueManager : MonoBehaviour
{
	...
	private void ReachBookLimit()
	{
		if(legendsManager.ReachedBookLimit())
		{
			for (int i = dialogueList.Count - 1; i >= 0 ; i--)
			{
				if (!legendsManager.LegendIsRegistred(dialogueList[i].characterOwner) && !dialogueList[i].characterOwner.nomadic)
				{
					dialogueList.RemoveAt(i);
				}
			}
